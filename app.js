...
// Embedded branches data (loaded from branches.json)
...
// Initialize
function init() {
  // Branches are already loaded from embedded data
...
function showError(message) {
  error.innerHTML = message; // Use innerHTML to support HTML formatting
...
// Show city selection when user clicks "Check Branch Availability"
...
// Get status class from availability
function getStatusClass(availability) {
  ...
  if (status === 'IN_STOCK' || status === 'AVAILABLE') {
    // Check if limited stock (you can adjust this logic)
...
// Display branches with automatic availability checking
async function displayBranchesWithAvailability(cityBranches) {
  const list = document.getElementById('branches-list');
  list.innerHTML = '';
  
  // Filter branches that have required data
  ...
  // Create branch items with checking state
  const branchItems = [];
  ...
  // Hide main loading, but keep individual branch checking states
  hideLoading();

  // Check availability for all branches (with delay to avoid rate limiting)
  for (let i = 0; i < branchItems.length; i++) {
    ...
    try {
      ...
      // Add click handler to show detailed availability
